  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setCourts(
          Array.isArray(resCourts.data.data.content)
            ? resCourts.data.data.content
            : []
        );
      } catch (err) {
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow p-6 mb-2">
            ...
            <div className="flex items-center text-yellow-500 mb-1">
              <span className="font-bold mr-1">
                ★ {businessLocation.rating || 4.8}
              </span>
              <span className="text-gray-600">
                ({businessLocation.reviews || 1247} đánh giá)
              </span>
            </div>
            ...
            <div className="text-gray-700 mb-2">
              {businessLocation.description ||
                "Trung tâm thể thao hiện đại với đầy đủ các loại sân và dịch vụ chất lượng cao"}
            </div>
          </div>
          {/* Tabs */}
          <div className="bg-white rounded-xl shadow p-4">
            ...
            <div>
                ...
                <div className="space-y-6">
                  {!Array.isArray(courts) || courts.length === 0 ? (
                    <div className="text-gray-500 text-center py-8">
                      Chưa có sân nào cho địa điểm này.
                    </div>
                  ) : (
                    courts.map((court) => (
                      <div
                        key={court.id}
                        className="flex flex-col md:flex-row bg-gray-50 rounded-lg shadow p-4 items-center md:items-stretch"
                      >
                        ...
                        <div className="flex-1 px-4 flex flex-col justify-between">
                          ...
                          <div className="flex gap-2 mt-2">
                            <button
                              className="bg-emerald-600 text-white px-4 py-2 rounded font-semibold flex items-center gap-2 hover:bg-emerald-700 transition"
                              onClick={() => {
                            ...
                            <button
                              className="border border-emerald-600 text-emerald-600 px-4 py-2 rounded font-semibold flex items-center gap-2 hover:bg-emerald-50 transition"
                              onClick={() => {
              ...
              {tab === "utilities" && (
                <div className="text-gray-500 text-center py-8">
                  Chức năng tiện ích đang phát triển...
                </div>
              )}
              {tab === "reviews" && (
                <div className="text-gray-500 text-center py-8">
                  Chức năng đánh giá đang phát triển...
                </div>
              )}
              {tab === "info" && (
                <div className="text-gray-500 text-center py-8">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <div className="text-gray-600 text-sm flex items-center gap-2">
              <span>
                Đóng cửa lúc{" "}
                {(businessLocation.closeTime || "23:00").slice(0, 5)}
              </span>
            </div>
            <button
              ...
              courts.slice(0, 2).map((court) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-emerald-600 text-white px-3 py-1 rounded font-semibold text-xs"
                  >
                    Đặt
                  </button>
                </div>
