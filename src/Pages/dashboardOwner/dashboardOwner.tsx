  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        {tab === "overview" && (
          <div className="text-lg">Chào mừng bạn đến với dashboard quản lý của chủ sân!</div>
        )}
        {tab === "location" && (
          <OwnerLocationTab
            onDetail={(record: BusinessLocation) => navigate(`/businessLocation/${record.id}`)}
          />
        )}
        {tab === "court" && (
          <OwnerCourtTab
            onDetail={(record: Court) => {/* TODO: điều hướng sang trang chi tiết sân nếu có */}}
          />
        )}
        {tab === "booking" && (
          <OwnerBookingTab
            onDetail={(record: Slot) => {/* TODO: điều hướng sang trang chi tiết booking nếu có */}}
          />
        )}
      </main>
    </div>
